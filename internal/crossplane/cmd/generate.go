...
func (cmd *GenerateCommand) Run(args []string) int {
	...
	cmd.xrdInputPath = fs.Arg(0)
	if cmd.xrdInputPath == "" {
		logger.Error("error executing command", "err", "XRD spec file is required as last argument")
		return 1
	}
...
func (cmd *GenerateCommand) runInternal(logger *slog.Logger) error {
	...
	oasBytes, config, err := loadOpenAPISchemaFromXRD(cmd.xrdInputPath)
	if err != nil {
		return fmt.Errorf("error reading OpenAPI spec file: %w", err)
	}
	println(string(oasBytes))
